...
INSERT INTO `bank`.`application` (`idApplication`, `Sum`, `CreditState`, `Currency`, `Client_idClient`) VALUES ('15', '6600', 'Not returned', 'Euro', '3');
  ...
  Where LastName Like '%OV' Or LastName Like '%OVA';
    ...
    Select * 
    From  department d
    Join client c on c.idClient = d.idDepartment
    where DepartmentCity='Kyiv';
    
    
	/*7. +Вивести імена клієнтів та їхні номера телефону, погрупувавши їх за іменами.  (В ні одній із таблиць немає поля номера телефону)*/
    ...
    join client c on c.idClient = a.idApplication
    where Sum > 5000 and CreditState = 'Not returned';
    
	/*9. +Порахувати кількість клієнтів усіх відділень та лише львівських відділень.*/
    select count(city) allClient
    from client;
	
	select count(city) LvivClient
    from client
    where City='Lviv';
	...
	Select  FirstName,LastName, max(Sum) maxCredits
    from application a
    join client c on c.idClient = a.Client_idClient
    group by Client_idClient;
   
	/*11. Визначити кількість заявок на крдеит для кожного клієнта.*/
	Select  idClient, FirstName, LastName, count(Client_idClient) 
    from client c
    ...
    select max(sum) MaxCredit, min(sum) MinCredit
    from application;
    
    
    /*13. Порахувати кількість кредитів для клієнтів,які мають вищу освіту.*/
	Select  idClient, FirstName, LastName, Education, count(Client_idClient) CountCredit
    from client c
    join application a on a.Client_idClient = c.idClient
    where Education = 'high'
    group by Client_idClient;
    ...
    Select c.*, Sum(Sum)/Count(Client_idClient) ser
    from application
...
Delete application from application, client
where idClient = Client_idClient
and substr(LastName,2,1) IN ('a','i','o','e','y','u');


/*Знайти львівські відділення, які видали кредитів на загальну суму більше ніж 5000*/

select d.*  from department d, client c, application a
...
Select a.* from application a, client c
where idClient = Client_idClient
and sum>(select Avg(sum) 
from application);
